...
public class Target {
    @FXML
    private ListView<Backuptargetroot> listView;

    public void getAll(){
        ObservableList<Backuptargetroot> all = BackuptargetrootDao.all();
    ...
    public void initialize() throws IOException {
        ...
        listView.setCellFactory(param -> new ListCell<>() {
            @Override
            protected void updateItem(Backuptargetroot item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
                    setText(null);
                } else {
                    setText(item.getTagetrootpath());
                }
            }
        });

        // 获取ListView的选择模型
        MultipleSelectionModel<Backuptargetroot> selectionModel = listView.getSelectionModel();
        // 监听选择改变事件
    ...
    private void addDir(){
        try {
            // 加载新建窗口的FXML文件
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/add_target_dialog.fxml"));
            Parent root = loader.load();

            // 获取窗口的控制器
            AddTargetDialog controller = loader.getController();
    ...
    private void editDir(){
        ...
        try {
            ...
            Backuptargetroot selectedItem = listView.getSelectionModel().getSelectedItem();
    ...
    private void deleteDir(){
        ...
        boolean delete = BackuptargetrootDao.delete(String.valueOf(selectedItem.getId()));
        if(delete){
            getAll();
        }
    }
